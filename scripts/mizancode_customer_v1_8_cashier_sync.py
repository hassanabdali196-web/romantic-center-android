...
# The cashier/cloud is now the only point source. Remove obsolete invoice scanner state and logic.
s=s.replace("  List<String> scannedInvoices = [];\n", "", 1)
s=re.sub(r"\n  Future<void> _loadInvoices\(\) async \{.*?\n  \}\n", "\n", s, count=1, flags=re.S)
s=re.sub(r"\n  Future<void> _scanInvoice\(\) async \{.*?\n  \}\n\n  Map<String, dynamic> _parseInvoice\(String raw\) \{.*?\n  \}\n", "\n", s, count=1, flags=re.S)

# Clear legacy scanned-invoice cache once and immediately sync from cloud.
anchor="""    _loadInvoices();\n    Future.microtask(() => _refreshCloud(silent: true));"""
replace="""    Future.microtask(() async {\n      final sp=await SharedPreferences.getInstance();\n      await sp.remove('scanned_invoice_ids');\n      await _refreshCloud(silent: true);\n    });"""
if anchor in s:
    s=s.replace(anchor,replace,1)
else:
    s=s.replace("""    scannedInvoices.clear();\n    Future.microtask(() async {""", """    Future.microtask(() async {""", 1)
